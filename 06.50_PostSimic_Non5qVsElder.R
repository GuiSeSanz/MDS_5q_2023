...
sc_data_PrePost_subseted <- readRDS('/home/tereshkova/data/gserranos/MDS/Data/pre_post_5q_Annotated_final.rds')
...
data_5q_info <- data_5q[[c('Cluster_names', 'Sample')]]
data_elder_info <- data_elder[[c('Cluster_names', 'Sample')]]
tmp <- rbind(data_5q_info, data_elder_info)

seurat_data <- tmp
seurat_data$Case <- ifelse(grepl('^SMD', seurat_data$Sample) , 'non5q', 'elder')
...
weights_file <- '/home/tereshkova/data/gserranos/MDS/Data/SimiC/Non5qVsElder_1000_L10.01_L20.01_Ws_filtered_BIC.pickle'
SimiC_weights <- py_load_object(filename =weights_file)
pd <- import("pandas")
AUCs_file <-'/home/tereshkova/data/gserranos/MDS/Data/SimiC/Non5qVsElder_1000_L10.01_L20.01_AUCs.pickle'
...
pdf(paste0('./Plots/SimiC/Non5qVsElder/Simic_Filtered_R2_targets_hist_Non5qVsElder.pdf'))
for (phenotype in phenotypes){
...
pdf(paste0('./Plots/SimiC/Non5qVsElder/Simic_TF_weigths_Non5qVsElder.pdf'), onefile = TRUE, width=20)
TF_2_remove <- c()
for(drv in unique(SimiC_weights_df$driver)){
  ...
  p <- ggplot(tmp_plotter_filtered, aes(x=target, y=value, fill=.id)) + 
    ...
    scale_fill_manual(values=c('#d9ed92', '#1e6091')) + 
...
# pdf(paste0('./Plots/SimiC/Simic_TF_stats_Non5qVsElder.pdf'), onefile = TRUE)
...
pdf(paste0('./Plots/SimiC/Non5qVsElder/Simic_Targets_weigths_Non5qVsElder.pdf'), onefile = TRUE, width=20)
plot_counter <- 1
plot_list <- list()
for(tgt in sort(as.character(unique(SimiC_weights_df$target)))){
  ...
  assign(paste0('p', plot_counter), ggplot(tmp_plotter, aes(x=reorder(driver, -abs(value)), y=value, fill=.id,  palette = "jco")) + 
    geom_bar(stat='identity', position='dodge', color='black') + 
    scale_fill_manual(values=c('#084c61', '#db3a34')) + 
...
# AUCs <- list()
# for(i in seq_along(phenotypes)){
# 	AUCs[[i]] <- read.table(paste0('./Data/SimiC/Results//Non5qVsElder_1000_L10.01_L20.01_AUCs_', assignment[[i]], '_BIS.csv'), header=T, sep='\t')
# }
...
pdf(paste0('./Plots/SimiC/Non5qVsElder/Simic_Population_phenotype_Non5qVsElder.pdf'))
  ggplot(tmp, aes(x=cluster_id, y=value, fill=TimePoint)) + 
  geom_bar(stat='identity' , position='dodge',  color='black') + 
  geom_text(aes(label=value), position=position_dodge(width=0.9), vjust=-0.25) +
   scale_fill_manual(values=c('#d9ed92', '#1e6091')) + theme_bw() + theme(axis.text.x = element_text(angle = 45, hjust=1, size=4))
dev.off()
...
for(cluster in clusters_2_keep){
  ...
  MinMax_val <- NULL
  for (tf in unique(df_auc_common$driver)[-100]){
    n_breaks = 100
    Wauc_dist <- list()
    for (phenotype in phenotypes){
  ...
  rownames(MinMax_val) <-  unique(df_auc_common$driver)[-100]
...
for (clust in clusters_2_keep){
  ...
  pdf(paste0('./Plots/SimiC/Non5qVsElder/Simic_Auc_Cluster_',clust,'_Non5qVsElder.pdf'), width = 15, onefile = TRUE)
  plot_counter <- 1
...
pdf(paste0('./Plots/SimiC/Non5qVsElder/Simic_Auc_all_cells_Non5qVsElder.pdf'), width = 15, onefile = TRUE)
plot_counter <- 1
plotter2 <- df_auc
for (tf in unique(plotter2$driver)){
	assign( paste0('p', plot_counter), 
...
# pdf(paste0('./Plots/SimiC/Simic_Auc_all_cells_perPatient_Non5qVsElder.pdf'), width = 15, onefile = TRUE)
# plot_counter <- 1
# for (tf in unique(plotter2$driver)){
# 	assign( paste0('p', plot_counter), 
# 	ggplot(plotter2[plotter2$driver ==tf,], aes(x=value, fill=TimePoint)) + 
...
pdf(paste0('./Plots/SimiC/Non5qVsElder/Simic_HeatMaps_RegDissScore_Non5qVsElder.pdf'), height = plot_dims$height, width = plot_dims$width )
print(p)
dev.off()
...
pdf(paste0('./Plots/SimiC/Non5qVsElder/Simic_tSNE_selection_Non5qVsElder.pdf'), height =8, width=18)
...
pdf('./Plots/SimiC/Non5qVsElder/Enrich_Regulons.pdf')
for(drv in sort(as.character(unique(SimiC_weights_df$driver)[-100]))){
  print(drv)
