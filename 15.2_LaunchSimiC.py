...
# SAMPLE = 'POST_Run_Normal_1000'
# SAMPLE = 'POST_Run_Del1_1000'
# SAMPLE = 'POST_Run_Del2_1000'
# SAMPLE = 'POST_Run_Del3_1000'
SAMPLE = 'POST_Run_Del_PREPOST_1000'

p2assignment = os.path.join(DATA_PATH, 'POST_Run_Del_PREPOST' +'.clustAssign.txt')
# We need the following files:
p2df = os.path.join(DATA_PATH, SAMPLE+'.DF.pickle')
p2tf = os.path.join(DATA_PATH, SAMPLE+'.TF.pickle')
assert os.path.exists(p2df), f'File not found: ' + p2df
assert os.path.getsize(p2df) > 0, f'File is empty: ' + p2df
assert os.path.exists(p2assignment), f'File not found: ' + p2assignment
assert os.path.getsize(p2assignment) > 0, f'File is empty: ' + p2assignment
assert os.path.exists(p2tf), f'File not found: ' + p2tf
assert os.path.getsize(p2tf) > 0, f'File is empty: ' + p2tf
