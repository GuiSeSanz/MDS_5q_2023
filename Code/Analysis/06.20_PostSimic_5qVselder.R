...
tmp <- tmp[tmp$Case == 'del5q', ]
tmp$Case  <- 'real_5q'

tmp <- rbind(tmp, data.frame(Case= 'elder', Cluster_name =elder_integrated_subseted$Cluster_names))
...
weights_file <- './Data/SimiC/Results/5qVsElder_1000_L10.01_L20.01_Ws_filtered_BIC.pickle'
SimiC_weights <- py_load_object(filename =weights_file)
pd <- import("pandas")
AUCs_file <-'./Data/SimiC/Results/5qVsElder_1000_L10.01_L20.01_AUCs.pickle'
...
pdf(paste0('./Plots/SimiC/5qVsElder/Simic_Filtered_R2_targets_hist.pdf'))
for (phenotype in phenotypes){
...
pdf(paste0('./Plots/SimiC/5qVsElder/Simic_TF_weigths.pdf'), onefile = TRUE, width=20)
TF_2_remove <- c()
for(drv in unique(SimiC_weights_df$driver)){
  ...
  p <- ggplot(tmp_plotter_filtered, aes(x=target, y=value, fill=.id)) + 
    ...
    scale_fill_manual(values=c(elder="#39A6A3", real_5q="#BF1363")) +
...
# pdf(paste0('./Plots/SimiC/5qVsElder/Simic_TF_stats.pdf'), onefile = TRUE)
...
pdf(paste0('./Plots/SimiC/5qVsElder/Simic_Targets_weigths.pdf'), onefile = TRUE, width=20)
plot_counter <- 1
plot_list <- list()
...
#   AUCs[[i]] <- py_to_r(aucs[[i]])
# }

AUCs <- list()
for(i in seq_along(phenotypes)){
	AUCs[[i]] <- read.table(paste0('./Data/SimiC/Results/5qVsElder_1000_L10.01_L20.01_AUCs_', assignment[[i]], '_BIS.csv'), header=T, sep='\t')
}
...
for(phenotype in phenotypes){
  ...
  # (stringr::str_extract(rownames(AUCs_cell_i), '(?<=_)[A-Z0-9-_]+$') %in% cell_names_phenotype)
	AUCs_by_state[[phenotype]] <- AUCs_cell_i[stringr::str_extract(rownames(AUCs_cell_i), '(?<=_)[A-Z0-9-_]+$') %in% cell_names_phenotype, ]
	state_specific_AUCs <- rbind(state_specific_AUCs, AUCs_cell_i[stringr::str_extract(rownames(AUCs_cell_i), '(?<=_)[A-Z0-9-_]+$') %in% cell_names_phenotype, ])
}

df <- do.call('rbind', AUCs_by_state)
df$cell_id <- stringr::str_extract(rownames(state_specific_AUCs), '(?<=_)[A-Z0-9-_]+$')
...
pdf(paste0('./Plots/SimiC/5qVsElder/Simic_Population_phenotype.pdf'))
  ggplot(tmp, aes(x=cluster_id, y=value, fill=TimePoint)) + 
  geom_bar(stat='identity' , position='dodge',  color='black') + 
  geom_text(aes(label=value), position=position_dodge(width=0.9), vjust=-0.25) +
  scale_fill_manual(values=c(elder="#39A6A3", real_5q="#BF1363")) + theme_bw() + theme(axis.text.x = element_text(angle = 45, hjust=1, size=4))
dev.off()
...
for (clust in clusters_2_keep){
  ...
  pdf(paste0('./Plots/SimiC/5qVsElder/Simic_Auc_Cluster_',clust,'.pdf'), width = 15, onefile = TRUE)
  plot_counter <- 1
...
pdf(paste0('./Plots/SimiC/5qVsElder/Simic_Auc_all_cells.pdf'), width = 15, onefile = TRUE)
plot_counter <- 1
plotter2 <- df_auc
for (tf in unique(plotter2$driver)){
	assign( paste0('p', plot_counter), 
...
# pdf(paste0('./Plots/SimiC/5qVsElder/Simic_Auc_all_cells_perPatient.pdf'), width = 15, onefile = TRUE)
# plot_counter <- 1
# for (tf in unique(plotter2$driver)){
# 	assign( paste0('p', plot_counter), 
# 	ggplot(plotter2[plotter2$driver ==tf,], aes(x=value, fill=TimePoint)) + 
...
pdf(paste0('./Plots/SimiC/5qVsElder/Simic_HeatMaps_RegDissScore.pdf'), height = plot_dims$height, width = plot_dims$width )
print(p)
dev.off()


# cannot be done as the two set of cells are not integrated
...
# pdf(paste0('./Plots/SimiC/5qVsElder/Simic_tSNE_selection.pdf'), height =8, width=18)
...
pb <- progress::progress_bar$new(total = length(unique(SimiC_weights_df$driver)))

pdf('./Plots/SimiC/5qVsElder/Enrich_Regulons.pdf')
for(drv in sort(as.character(unique(SimiC_weights_df$driver)))){
  ...
  print(cowplot::plot_grid(title_theme, 
        cowplot::plot_grid(clusterProfiler::dotplot(results_BP)+ theme_mod, 
                           clusterProfiler::dotplot(results_MF) + theme_mod, 
                           clusterProfiler::dotplot(results_CC) + theme_mod, 
                           clusterProfiler::dotplot(results_KG) + theme_mod, 
