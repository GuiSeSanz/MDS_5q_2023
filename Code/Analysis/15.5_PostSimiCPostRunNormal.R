...
tmp <- rbind(FetchData(post_data  , vars=c('Cluster_names', 'cell_5q', 'Sample')),
             ...
             FetchData(elder_data , vars=c('Cluster_names', 'cell_5q', 'Sample')))
tmp$cell_id <- rownames(tmp)

tmp$Case <- ifelse(tmp$cell_id %in% colnames(elder_data), 'Elder',
					  ifelse(tmp$cell_id %in% colnames(MDS_data), 'Diagnosed', 
            ifelse(tmp$Sample == 'FS-0406-post', 'PR', 'CR')))
seurat_data <- tmp
...
# # 2 <- PR
# # 3 <- CR
phenotypes <- c('Elder', 'Diagnosed', 'PR', 'CR')
color_map <- c("#f94144","#f9c74f","#4d908e","#277da1")
...
weights_file <- './Data/SimiC/Results/POST_Run_Normal_1000_L10.01_L20.01_Ws_filtered_BIC.pickle'
SimiC_weights <- py_load_object(filename =weights_file)
pd <- import("pandas")
AUCs_file <-'./Data/SimiC/Results/POST_Run_Normal_1000_L10.01_L20.01_AUCs.pickle'
...
pdf(paste0('./Plots/SimiC/Post/Normal/Simic_Filtered_R2_targets_hist_PostNormal.pdf'))
for (phenotype in phenotypes){
...
pdf(paste0('./Plots/SimiC/Post/Normal/Simic_TF_weigths_PostNormal.pdf'), onefile = TRUE, width=20)
TF_2_remove <- c()
...
# pdf(paste0('./Plots/SimiC/Simic_TF_stats_PostNormal.pdf'), onefile = TRUE)
...
pdf(paste0('./Plots/SimiC/Post/Normal/Simic_Targets_weigths_PostNormal.pdf'), onefile = TRUE, width=20)
plot_counter <- 1
plot_list <- list()
...
# AUCs <- list()
# for(i in seq_along(phenotypes)){
# 	AUCs[[i]] <- read.table(paste0('./Data/SimiC/Results//PostNormal_1000_L10.01_L20.01_AUCs_', assignment[[i]], '_BIS.csv'), header=T, sep='\t')
# }
...
pdf(paste0('./Plots/SimiC/Post/Normal/Simic_Population_phenotype_PostNormal.pdf'))
...
for (clust in clusters_2_keep){
  ...
  pdf(paste0('./Plots/SimiC/Post/Normal/Simic_Auc_Cluster_',clust,'_PostNormal.pdf'), width = 15, onefile = TRUE)
  plot_counter <- 1
...
saveRDS(df_auc, '/home/tereshkova/data/gserranos/MDS/Data/SimiC_df_auc_PostNormal.rds')

pdf(paste0('./Plots/SimiC/Post/Normal/Simic_Auc_all_cells_PostNormal.pdf'), width = 15, onefile = TRUE)
plot_counter <- 1
plotter2 <- df_auc
for (tf in unique(plotter2$driver)){
	message(tf)
...
# pdf(paste0('./Plots/SimiC/Simic_Auc_all_cells_perPatient_PostNormal.pdf'), width = 15, onefile = TRUE)
# plot_counter <- 1
# for (tf in unique(plotter2$driver)){
# 	assign( paste0('p', plot_counter), 
# 	ggplot(plotter2[plotter2$driver ==tf,], aes(x=value, fill=TimePoint)) + 
...
saveRDS(MinMax_clust, '/home/tereshkova/data/gserranos/MDS/Data/SimiC_MinMax_clust_PostNormal.rds')
...
pdf(paste0('./Plots/SimiC/Post/Normal/Simic_HeatMaps_RegDissScore_PostNormal.pdf'), height = plot_dims$height, width = plot_dims$width )
print(p)
dev.off()
...
# pdf(paste0('./Plots/SimiC/Post/Normal/Simic_tSNE_selection_PostNormal.pdf'), height =8, width=18)
...
saveRDS(SimiC_weights_df,  '/home/tereshkova/data/gserranos/MDS/Data/SimiC_Weigths_PostNormal.rds')
saveRDS(unselected_targets, '/home/tereshkova/data/gserranos/MDS/Data/SimiC_unselected_targets_PostNormal.rds')

pdf('./Plots/SimiC/Post/Normal/Enrich_Regulons_PostNormal.pdf')
