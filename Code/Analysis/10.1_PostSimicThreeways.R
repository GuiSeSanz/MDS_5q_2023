...
cell_5q <- data_5q[['cell_5q']]
cell_5q$cel_name <- rownames(cell_5q)
cell_5q <- cell_5q[cell_5q$cell_5q == 'del5q', 'cel_name']
data_elder <- readRDS('/home/tereshkova/data/gserranos/MDS/Data/elder_Samples_Annotated_final.rds')

tmp <- rbind(data_5q[[c('Cluster_names', 'Sample')]], data_elder[[c('Cluster_names', 'Sample')]])
tmp$cell_id <- rownames(tmp)

tmp$Case <- ifelse(tmp$cell_id %in% colnames(data_elder), 'elder',
					ifelse(tmp$cell_id %in% cell_5q, '5q', 'non5q'))
seurat_data <- tmp
...
weights_file <- './Data/SimiC/Results/ThreeWayComparison_1000_L10.01_L20.01_Ws_filtered_BIC.pickle'
SimiC_weights <- py_load_object(filename =weights_file)
pd <- import("pandas")
AUCs_file <-'./Data/SimiC/Results/ThreeWayComparison_1000_L10.01_L20.01_AUCs.pickle'
...
pdf(paste0('./Plots/SimiC/ThreeWay/Simic_Filtered_R2_targets_hist_ThreeWay.pdf'))
for (phenotype in phenotypes){
...
pdf(paste0('./Plots/SimiC/ThreeWay/Simic_TF_weigths_ThreeWay.pdf'), onefile = TRUE, width=20)
TF_2_remove <- c()
...
# pdf(paste0('./Plots/SimiC/Simic_TF_stats_ThreeWay.pdf'), onefile = TRUE)
...
pdf(paste0('./Plots/SimiC/ThreeWay/Simic_Targets_weigths_ThreeWay.pdf'), onefile = TRUE, width=20)
plot_counter <- 1
plot_list <- list()
...
# AUCs <- list()
# for(i in seq_along(phenotypes)){
# 	AUCs[[i]] <- read.table(paste0('./Data/SimiC/Results//ThreeWay_1000_L10.01_L20.01_AUCs_', assignment[[i]], '_BIS.csv'), header=T, sep='\t')
# }
...
pdf(paste0('./Plots/SimiC/ThreeWay/Simic_Population_phenotype_ThreeWay.pdf'))
...
for (clust in clusters_2_keep){
  ...
  pdf(paste0('./Plots/SimiC/ThreeWay/Simic_Auc_Cluster_',clust,'_ThreeWay.pdf'), width = 15, onefile = TRUE)
  plot_counter <- 1
...
saveRDS(df_auc, '/home/tereshkova/data/gserranos/MDS/Data/SimiC_df_auc_ThreeWay.rds')

pdf(paste0('./Plots/SimiC/ThreeWay/Simic_Auc_all_cells_ThreeWay.pdf'), width = 15, onefile = TRUE)
plot_counter <- 1
plotter2 <- df_auc
for (tf in unique(plotter2$driver)){
	message(tf)
...
# pdf(paste0('./Plots/SimiC/Simic_Auc_all_cells_perPatient_ThreeWay.pdf'), width = 15, onefile = TRUE)
# plot_counter <- 1
# for (tf in unique(plotter2$driver)){
# 	assign( paste0('p', plot_counter), 
# 	ggplot(plotter2[plotter2$driver ==tf,], aes(x=value, fill=TimePoint)) + 
...
saveRDS(MinMax_clust, '/home/tereshkova/data/gserranos/MDS/Data/SimiC_MinMax_clust_ThreeWay.rds')
...
pdf(paste0('./Plots/SimiC/ThreeWay/Simic_HeatMaps_RegDissScore_ThreeWay.pdf'), height = plot_dims$height, width = plot_dims$width )
print(p)
dev.off()
...
# pdf(paste0('./Plots/SimiC/ThreeWay/Simic_tSNE_selection_ThreeWay.pdf'), height =8, width=18)
...
saveRDS(SimiC_weights_df,  '/home/tereshkova/data/gserranos/MDS/Data/SimiC_Weigths_ThreeWay.rds')
saveRDS(unselected_targets, '/home/tereshkova/data/gserranos/MDS/Data/SimiC_unselected_targets_ThreeWay.rds')

pdf('./Plots/SimiC/ThreeWay/Enrich_Regulons_ThreeWay.pdf')
