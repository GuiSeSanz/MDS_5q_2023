...
plot_stats <- function(obj, pdf_name){
  pdf(paste0(PLOT_PATH, pdf_name,'.pdf'))
	...
	if('doublet' %in% colnames(obj@meta.data)){
		plt <- obj@meta.data[, c('nUMI', 'nGene', 'doublet')]
		plt <- plt[order(plt$doublet),]
		print(ggplot(plt, aes(x=nUMI, y=nGene, color=doublet)) + 
        geom_point() + 
        scale_colour_manual(values=c(False = "gray90", True = "red")) +
        stat_smooth(method=lm) +
        scale_x_log10() + 
        scale_y_log10() + 
        theme_classic() +
...
PLOT_PATH <- paste0(getwd(), '/Plots/',SAMPLE,'/')
dir.create(PLOT_PATH, showWarnings = FALSE)

data_flt <- Read10X(data.dir = paste0('/home/tereshkova/data/gserranos/MDS/Data/',SAMPLE,'/outs/filtered_feature_bc_matrix/'))

colnames(data_flt) <- paste0(SAMPLE, '_', colnames(data_flt))
seurat_obj <- CreateSeuratObject(counts = data_flt, min.features = 100, project = '5q')
...
metadata <- setNames(seurat_obj@meta.data, c('orig.ident', 'nUMI', 'nGene' ,'log10GenesPerUMI', 
											'mitoRatio', 'RPSRatio', 'Phenotype', 'Sample'))
seurat_obj@meta.data <- metadata
message('Prefilter plot')
if(file.exists(paste0('/home/tereshkova/data/gserranos/MDS/Data/',SAMPLE,'/outs/filtered_feature_bc_matrix/scrublet_output_table.csv'))){
	doublets <- read.table(paste0('/home/tereshkova/data/gserranos/MDS/Data/',SAMPLE,'/outs/filtered_feature_bc_matrix/scrublet_output_table_cellID.csv'), sep=',', header=TRUE)
	doublets$barcode <- paste0(SAMPLE, '_', doublets$barcode)
	doublets$predicted_doublet <- ifelse(doublets$score > 0.1, 'True', 'False')
}

seurat_obj$doublet <- doublets[doublets$barcode %in% colnames(seurat_obj), 'predicted_doublet']
...
if(file.exists(paste0('/home/tereshkova/data/gserranos/MDS/Data/',SAMPLE,'/outs/filtered_feature_bc_matrix/scrublet_output_table.csv'))){
	filtered_seurat <- subset(x = seurat_obj, 
						...
						(nGene <= 7000) & # discard cells with more than 5000 genes (duplets or triplets)
...
saveRDS(filtered_seurat, paste0(getwd(), '/Data/', SAMPLE, '/', SAMPLE, '_seurat_obj_norm.rds'))
