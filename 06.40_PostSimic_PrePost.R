...
assigmnet<- readRDS('./Data/SelectedCells5q_PrePost_CASPER_COPYKAT.rds')
tmp <- sc_data_PrePost_subseted[[c('Cluster_names', 'Sample')]]
tmp$cell_id <- rownames(tmp)
tmp <- tmp[tmp$cell_id %in% assigmnet,]
tmp$Case <- ifelse(tmp$Sample == 'SMD211420', 'Pre', 'Post')
seurat_data <- tmp
...
weights_file <- './Data/SimiC/Results/PreVsPost_1000_L10.01_L20.01_Ws_filtered_BIC.pickle'
SimiC_weights <- py_load_object(filename =weights_file)
pd <- import("pandas")
AUCs_file <-'./Data/SimiC/Results/PreVsPost_1000_L10.01_L20.01_AUCs.pickle'
...
pdf(paste0('./Plots/SimiC/PrePost/Simic_Filtered_R2_targets_hist_PreVsPost.pdf'))
for (phenotype in phenotypes){
...
pdf(paste0('./Plots/SimiC/PrePost/Simic_TF_weigths_PreVsPost.pdf'), onefile = TRUE, width=20)
TF_2_remove <- c()
for(drv in unique(SimiC_weights_df$driver)){
  ...
  p <- ggplot(tmp_plotter_filtered, aes(x=target, y=value, fill=.id)) + 
    ...
    scale_fill_manual(values=c('#007f5f', '#dddf00')) + 
...
# pdf(paste0('./Plots/SimiC/Simic_TF_stats_PreVsPost.pdf'), onefile = TRUE)
...
pdf(paste0('./Plots/SimiC/PrePost/Simic_Targets_weigths_PreVsPost.pdf'), onefile = TRUE, width=20)
plot_counter <- 1
plot_list <- list()
...
# AUCs <- list()
# for(i in seq_along(phenotypes)){
# 	AUCs[[i]] <- read.table(paste0('./Data/SimiC/Results//PreVsPost_1000_L10.01_L20.01_AUCs_', assignment[[i]], '_BIS.csv'), header=T, sep='\t')
# }
...
pdf(paste0('./Plots/SimiC/PrePost/Simic_Population_phenotype_PreVsPost.pdf'))
  ggplot(tmp, aes(x=cluster_id, y=value, fill=TimePoint)) + 
  geom_bar(stat='identity' , position='dodge',  color='black') + 
  geom_text(aes(label=value), position=position_dodge(width=0.9), vjust=-0.25) +
  scale_fill_manual(values=c("#007f5f","#dddf00")) + theme_bw() + theme(axis.text.x = element_text(angle = 45, hjust=1, size=4))
dev.off()
...
for (clust in clusters_2_keep){
  ...
  pdf(paste0('./Plots/SimiC/PrePost/Simic_Auc_Cluster_',clust,'_PreVsPost.pdf'), width = 15, onefile = TRUE)
  plot_counter <- 1
...
pdf(paste0('./Plots/SimiC/PrePost/Simic_Auc_all_cells_PreVsPost.pdf'), width = 15, onefile = TRUE)
plot_counter <- 1
plotter2 <- df_auc
for (tf in unique(plotter2$driver)){
	assign( paste0('p', plot_counter), 
...
# pdf(paste0('./Plots/SimiC/Simic_Auc_all_cells_perPatient_PreVsPost.pdf'), width = 15, onefile = TRUE)
# plot_counter <- 1
# for (tf in unique(plotter2$driver)){
# 	assign( paste0('p', plot_counter), 
# 	ggplot(plotter2[plotter2$driver ==tf,], aes(x=value, fill=TimePoint)) + 
...
pdf(paste0('./Plots/SimiC/PrePost/Simic_HeatMaps_RegDissScore_PreVsPost.pdf'), height = plot_dims$height, width = plot_dims$width )
print(p)
dev.off()
...
# pdf(paste0('./Plots/SimiC/PrePost/Simic_tSNE_selection_PreVsPost.pdf'), height =8, width=18)
...
pdf('./Plots/SimiC/PrePost/Enrich_Regulons_PrePost.pdf')
